...
* Filters
*/
$(document).on('input' , '#product-filter , #product-filter-pur' , function (){
    var name = $(this).val();
    $.ajax({
        ...
        success:function(data){
            getItems(data ,'#product-search-pur ,#product-search');
            if (data.length == 1) {
...
$(document).ready(function() {
    $(document).on('change' , '#product-search-pur' , function (e){
        getItem($('#product-search-pur').val() ,"/product/purchase/filter")
    });
});
...
$(document).ready(function() {
    $(document).on('blur' , '#customer_id , #customer_name' , function (e){
        e.preventDefault();
        var id = $(this).val();
        $.ajax({
            url:"/customer/filter",
...
$(document).on('change' , '#product-type' , function (e){
    ...
    if (window.location.href.indexOf("purchase") != -1){
        url = '/product/pur/type/filter';
    }else{
        url = '/product/sales/type/filter';
    }
