...
class PosReturnReceiptService{
     ...
     * @throws \Exception
     */
    public function create($view = "pos.create" ,$invoice_type = Transaction::POS_RETURN_RECEIPT ){
        return view($view ,$this->returnInvoiceData($invoice_type) );
    }


    public function store($request ,$invoice_type =  Transaction::POS_RETURN_RECEIPT ) : void{
    ...
    public function edit($uuid ,$view,$invoice_type =  Transaction::POS_RETURN_RECEIPT){
    ...
    public function update($uuid ,$request ,$invoice_type =  Transaction::POS_RETURN_RECEIPT) : void {
     ...
     * @throws \Exception
     */
    private function getInvoiceID($invoice_type =  Transaction::POS_RETURN_RECEIPT) : string{
    ...
    private function returnSalesInvoiceHeadData($uuid ,$invoice_type ,$request  ,$status = 'Valid') :array{
        ...
        return [
            ...
            'invoice_date' => $request->invoice_date ?? date("Y-m-d\TH:i", strtotime(\Carbon\Carbon::now('Africa/Cairo'))),
    ...
    private function storeInvoiceItems($request,$uuid , $invoice_type ){
        ...
        foreach($request->items as  $item){
            ...
            DB::table('invoicedetails')->insert([
                ...
                'total' => $item['total'],
                'number' => $item['number'],
                'created_at' => $request->invoice_date ?? date("Y-m-d\TH:i", strtotime(\Carbon\Carbon::now('Africa/Cairo')))
            ]);
