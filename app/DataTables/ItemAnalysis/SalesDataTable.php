...
class SalesDataTable extends DataTable
{
    ...
    private function Q(){
        ...
        return DB::table('invoicedetails')
                ...
                DB::raw('CASE WHEN invoicedetails.invoice_type = 2 THEN SUM(invoicedetails.total_sales) / SUM(invoicedetails.qty) END as UNIT_PRICE'),
                DB::raw('CASE WHEN invoicedetails.invoice_type = 2 THEN SUM(invoicedetails.total_sales) END as TOTAL_SALES'),
                ...
                DB::raw('CASE WHEN invoicedetails.invoice_type = 2 THEN SUM(invoicedetails.total) END as VALUE')) //, CASE WHEN invoice_type = 2 THEN SUM(total) END as INCOME
                ->leftJoin('invoicehead' , 'invoicehead.uuid' ,'=','invoicedetails.uuid')
                ->leftJoin('products' ,'products.uuid' ,'=' ,'invoicedetails.item_uuid')
                ->where('invoicedetails.invoice_type' ,'2')
