...
use App\Exports\Reports\Supplier\SupplierCardExport;
...
class SupplierCardDataTable extends DataTable
{

    protected string $exportClass = SupplierCardExport::class;
    protected string $printPreview = 'print.reps.supplier.card';

    private function Q(){
        return DB::table('receipts')
        ->select('receipts.receipt_date','receipts.statement','receipts.receiver_name',
            DB::raw('CASE WHEN receipts.receipt_type IN(4,9) THEN value END as credit, CASE WHEN receipts.receipt_type IN (2,6,10)THEN value END as debit'),
            DB::raw('CONCAT_WS(" ", receipts.supplier_name , receipts.receiver_name)  as receiver'),
            DB::raw('SUM(CASE WHEN receipts.receipt_type IN (2,6,10) THEN value ELSE 0 END - CASE WHEN receipts.receipt_type IN (4,9) THEN value ELSE 0 END ) OVER (ORDER BY receipts.receipt_date ,receipts.id) balance'))
        ->leftJoin('invoicehead' ,'invoicehead.uuid' , '=' ,'receipts.uuid')
        ->whereIn('receipt_type' ,[2,4,6,9,10])
...
//                    ->dom('Bfrtip')
    ...
    {
        return 'SupplierCard_' . date('YmdHis');
    }
}
