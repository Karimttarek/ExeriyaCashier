...
use App\Exports\Reports\Supplier\SupplierItemsWithdrawalsExport;
...
{
    protected string $exportClass = SupplierItemsWithdrawalsExport::class;
    protected string $printPreview = 'print.reps.supplier.itemsWithdrawals';

    private function Q(){
        return DB::table('invoicehead')
            ->where('invoicehead.invoice_type' , 1)
            ->whereNotIn('invoicehead.status', ['Invalid','Canceled'])
            ->select('invoicehead.invoice_date','invoicehead.invoice_number','invoicehead.issuer_id' ,'invoicehead.issuer_name','invoicehead.issuer_street',
    ...
    public function dataTable()
    {
        ...
        return datatables($query)
                ...
                // })
                ->filterColumn('issuer_name', function ($query , $keyword) {
                    $sql = "invoicehead.issuer_name like ?";
    ...
    {
        return 'SupplierItemWithdrawal_' . date('YmdHis');
    }
}
